...
if(settings.nodeFillSapce&&node.fillAlpha>0){ctx.beginPath();ctx.moveTo(node.currentX,node.currentY);ctx.lineTo(node.Closest[i].currentX,node.Closest[i].currentY);ctx.lineTo(node.Closest[(i+1)%node.Closest.length].currentX,node.Closest[(i+1)%node.Closest.length].currentY);if(settings.nodeFillGradientColor!==null){var gradient=ctx.createLinearGradient(node.currentX,node.currentY,node.Closest[i].currentX,node.Closest[i].currentY);gradient.addColorStop(0,'rgba('+settings.nodeFillColor+','+node.fillAlpha+')');gradient.addColorStop(1,'rgba('+settings.nodeFillGradientColor+', '+node.fillAlpha+')');ctx.fillStyle=gradient}else{ctx.fillStyle='rgba('+settings.nodeFillColor+','+node.fillAlpha+')'}
ctx.fill()}}}
